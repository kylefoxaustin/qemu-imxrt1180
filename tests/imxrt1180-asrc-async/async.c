 ...
 *
 * so the true ratio is 2*1 = 2 -- a 1:2 up-conversion driven PURELY by the
 * source-clock ratio.  We feed a linear ramp x[n] = 100n and check the OUTPUT
 * against the exact up-converted ramp: a correct (normalized, symmetric) resampler
 * reproduces a straight line exactly in its interior, so at ratio 2 the output at
 * frame m is 100*(m/2) = 50*m, byte-exact.
 *
 * A model that ignores the source factor computes ratio = 1 (equal dividers) and
 * copies the input through -- out[m] = 100*m -- so the very first interior sample
 * (50*m vs 100*m) fails.  Both SAIs are mastered off the always-on 24 MHz RC
 * oscillator (CCM root mux slot 0), so no PLL bring-up is needed and MCLK cancels.
 *
...
#define NFRAMES  40                /* input ramp frames */
...
void reset_handler(void)
{
    ...
    /* ASRC pair A, mono: input = SAI1_TX (sel 0), output = SAI2_TX (sel 2). */
    R(ASRCSR)  = (2u << 12) | 0u;  /* AOCSA=2 (SAI2_TX), AICSA=0 (SAI1_TX) */
    R(ASRCNCR) = 1;                /* 1 channel */
    R(ASRCDR1) = 0x00;             /* in_div=out_div=1 -> divider ratio 1 */
    R(ASRCTR)  = 1u | (1u << 20);  /* enable + ATSA (init pair A) */

    for (int i = 0; i < NFRAMES; i++) {
        R(ASRDIA) = (uint32_t)(100 * i);         /* ramp x[n] = 100n */
    }

    /* At ratio 2 the interior output at frame m is 50*m (up-converted ramp). */
    int ok = 1, m = 0, guard = 0;
    while (m < 30) {
        if (!(R(ASRSTR) & AODFA)) { if (++guard > 200000) { ok = 0; break; } continue; }
        int32_t s = (int32_t)(R(ASRDOA) << 8) >> 8;   /* 24-bit signed */
        if (m >= 12 && m < 28 && s != 50 * m) { ok = 0; }   /* steady interior */
        m++;
    }

    if (ok && m >= 28) {
        puts_("ASRC-ASYNC: PASS - 1:2 up-conversion driven by the SAI2/SAI1 clock-source "
              "ratio (equal dividers), interior output is the exact up-converted ramp\r\n");
    } else {
        puts_("ASRC-ASYNC: FAIL - the true-async source-clock ratio was not applied "
              "(output ramp has the wrong rate)\r\n");
    }
