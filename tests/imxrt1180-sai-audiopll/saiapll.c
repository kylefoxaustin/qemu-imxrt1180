...
#define ROOT_MUX_AUDIOPLL 2u          /* SAI1 mux slot 2 = AudioPllOut (fsl_clock.h) */

/* AUDIO_PLL (PLL_Type @ 0x4448_4280): a fractional PLL configured exactly as the
 * stock hardware_init.c does (loopDivider 32, num 768, denom 1000, post 1) ->
 * 24e6*(32+768/1000)/2 = 393.216 MHz.  DIV_SELECT/POST_DIV_SEL are set via the
 * CTRL0 CLR-then-SET aliases; NUMERATOR/DENOMINATOR via their .RW registers. */
#define AUDIO_PLL   0x44484280u
#define APLL_CTRL0_RW  (*(volatile uint32_t *)(AUDIO_PLL + 0x00))
#define APLL_CTRL0_SET (*(volatile uint32_t *)(AUDIO_PLL + 0x04))
#define APLL_CTRL0_CLR (*(volatile uint32_t *)(AUDIO_PLL + 0x08))
#define APLL_NUMER_RW  (*(volatile uint32_t *)(AUDIO_PLL + 0x20))
#define APLL_DENOM_RW  (*(volatile uint32_t *)(AUDIO_PLL + 0x30))
#define APLL_DIV_SELECT_MASK 0x7Fu
#define APLL_POST_DIV_MASK   (0x7u << 25)
...
void reset_handler(void)
{
    ...
    /*
     * Bring up the AUDIO PLL exactly as the stock hardware_init.c does, then feed
     * SAI1's MCLK from it -- the whole point of this test.  Before this, CLK_AUDIO_PLL
     * returned 0 and a SAI1 muxed here got MCLK=0 (an honest stall).  393.216 MHz.
     */
    APLL_DENOM_RW = 1000u;
    APLL_NUMER_RW = 768u;
    APLL_CTRL0_CLR = APLL_DIV_SELECT_MASK;
    APLL_CTRL0_SET = 32u;                 /* DIV_SELECT (loopDivider) = 32 */
    APLL_CTRL0_CLR = APLL_POST_DIV_MASK;
    APLL_CTRL0_SET = (1u << 25);          /* POST_DIV_SEL = 1 -> divide by 2 */

    /* SAI1 MCLK = AudioPllOut / 16 = 24.576 MHz.  root 65, mux 2 (AudioPll), div 16. */
    CCM_ROOT_CTRL(CCM_ROOT_SAI1) = (ROOT_MUX_AUDIOPLL << 8) | (16u - 1u);
