...
#define LPSPI1 0x42550000u  /* actually LPSPI3, eDMA4 */
...
#define EDMA3_CH(n)  (0x42010000u + (n) * 0x8000u)  /* eDMA4 */
...
#define SRC_LPSPI1_TX 12u
#define SRC_LPSPI1_RX 13u
...
void reset_handler(void)
{
    ...
    if (TCD_SADDR(TX_CH) != (uint32_t)txbuf) {
        puts_("SPIDMA3: FAIL - NEG-TX: eDMA clocked frames with DER[TDDE] CLEAR\r\n");
        fail = 1;
    } else if (TCD_CITER(TX_CH) != NFRAME) {
        puts_("SPIDMA3: FAIL - NEG-TX: TX CITER advanced with DER[TDDE] CLEAR\r\n");
        fail = 1;
    }
    CH_CSR(TX_CH) = 0;
    CH_CSR(RX_CH) = 0;
    ...
    if (!(CH_CSR(RX_CH) & CH_CSR_DONE)) {
        puts_("SPIDMA3: FAIL - POS: RX channel never completed its major loop\r\n");
        fail = 1;
    } else if (!(CH_CSR(TX_CH) & CH_CSR_DONE)) {
        puts_("SPIDMA3: FAIL - POS: TX channel never completed its major loop\r\n");
        fail = 1;
    } else if (CH_CSR(TX_CH) & CH_CSR_ERQ) {
        puts_("SPIDMA3: FAIL - POS: TCD_CSR[DREQ] did not clear TX ERQ at completion\r\n");
        fail = 1;
    } else {
        for (unsigned i = 0; i < NFRAME; i++) {
            if ((rx_dma[i] & 0xFF) != GOLD[i]) {
                puts_("SPIDMA3: FAIL - POS: DMA-read JEDEC bytes are not the at25df321a ID\r\n");
                fail = 1;
                break;
            }
        }
    ...
    if (!(CH_CSR(TX_CH) & CH_CSR_DONE)) {
        puts_("SPIDMA3: FAIL - NEG-RX: TX channel never completed (TX gate stuck?)\r\n");
        fail = 1;
    }
    for (unsigned i = 0; i < NFRAME; i++) {
        if ((rx_dma[i] & 0xFF) != SENTINEL) {
            puts_("SPIDMA3: FAIL - NEG-RX: RX channel moved data with DER[RDDE] CLEAR\r\n");
            fail = 1;
            break;
        }
    }
    ...
    if (!fail) {
        puts_("SPIDMA3: PASS - TDDE gate refuses, RDDE gate refuses, both open => "
