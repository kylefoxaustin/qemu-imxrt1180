...
| **WM8962** (audio codec) | 1 | LPI2C2 @0x1A | ✅ control-plane | EVK on-board audio codec (`hw/audio/wm8962.c`). I2C register file — 2-byte reg address + 16-bit big-endian value, per `fsl_wm8962.c`; writes read back so `WM8962_Init`'s RMWs work, and the write-sequencer BUSY bit (0x5D bit0) always reads 0 so `WM8962_StartSequence`'s poll can't hang. **Control-plane ONLY**: on the EVK the WM8962 is the I2S *slave*, so it never touches the sample stream (the SAI's wav *is* the DAC output) — modelling it as a data-path device would be the fabrication. Mutation-proven (`tests/imxrt1180-wm8962`). Unblocks `CODEC_Init` for the stock SAI demo (still needs the Audio PLL for a non-zero SAI1 MCLK) |
