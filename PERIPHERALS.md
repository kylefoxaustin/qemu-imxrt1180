...
| **CCM** (clocks) | 1 | 0x44450000 | ✅ functional | **real clock tree**: `root_hz = source(CLOCK_ROOT[n].MUX) / (DIV+1)`, PLL/OSC frequencies computed from ANADIG as `CLOCK_GetPllFreq()` does; the 74-root mux table is taken mechanically from `fsl_clock.c`. All six timer blocks read it at the point of use. LPCG STATUS0 mirrors DIRECT.ON. OBSERVE.FREQUENCY_CURRENT is **computed** (it used to return a fabricated 6 MHz into guest arithmetic). **AUDIO PLL now computed** — `XTAL*(DIV_SELECT+NUM/DENOM)/2^POST_DIV` from the `AUDIO_PLL` PLL_Type registers (ANADIG+0x4280, whose CTRL0 SET/CLR/TOG aliases the ANADIG model now applies); value-verified via a SAI streaming off it (`tests/imxrt1180-sai-audiopll`, byte-exact at 48k/24k, formula mutation-proven). Reset values are the RM's cold-POR column. Gate: `tests/imxrt1180-clocktree` |
...
| **LPI2C** (controller mode) | 1..4 | 0x44340000, 0x44350000, 0x42530000, 0x42540000 | ✅ functional | command-FIFO master (START/TX/RX/STOP) on a real QEMU I2CBus — devices attach; MSR flags + NDF NACK detect + IRQ (13/14/62/63). **eDMA** request lines on LPI2C1/2 (`MDER[TDDE]` command-stream TX + `MDER[RDDE]`/`MFCR[RXWATER]` RX, both gates mutation-proven — `tests/imxrt1180-lpi2c-dma`) |
| **LPSPI** (controller mode) | 1..4 | 0x44360000, 0x44370000, 0x42550000, 0x42560000 | ✅ functional | full-duplex SPI master (TCR frame/PCS/CONT, TDR→SSIBus→RDR) with per-CS lines; validated vs a serial-flash JEDEC-ID read (PIO and **eDMA**: `DER[TDDE]` TX + `DER[RDDE]`/`FCR[RXWATER]` RX request lines on LPSPI1/2, both gates mutation-proven); IRQ (16/17/65/66). CS not wired to `-device` slaves → one command/boot, see the DMA-request note |
...
| **eFlexPWM** | 1..4 | 0x42650000, 0x42660000, 0x42670000, 0x42680000 | ◐ functional | Motor-control PWM. 4 submodules/module; INIT+VAL0..5 double-buffered (commit on MCTRL.LDOK); MCTRL.RUN starts a ptimer-backed reload at modulo/(clk/prescaler); STS.RF + INTEN.RIE raise the submodule reload IRQ (the FOC loop clock); PWMA duty computed (per-mille). SM0..3 IRQ 24-27/171-174/176-179/181-184 + fault 23/170/175/180. **eDMA** value-DMA request per submodule (`DMAEN[VALDE]` → eDMA4 SRC 62/70/78/86+sm; asserted at reload, lowered by the serviced VALx write, mutation-proven — `tests/imxrt1180-pwm-dma`). No motor plant/encoder responds + ADC-sync XBAR routing not modelled (flagged); capture-DMA not modelled |
...
| **WM8962** (audio codec) | 1 | LPI2C2 @0x1A | ✅ control-plane | EVK on-board audio codec (`hw/audio/wm8962.c`). I2C register file — 2-byte reg address + 16-bit big-endian value, per `fsl_wm8962.c`; writes read back so `WM8962_Init`'s RMWs work, and the write-sequencer BUSY bit (0x5D bit0) always reads 0 so `WM8962_StartSequence`'s poll can't hang. **Control-plane ONLY**: on the EVK the WM8962 is the I2S *slave*, so it never touches the sample stream (the SAI's wav *is* the DAC output) — modelling it as a data-path device would be the fabrication. Mutation-proven (`tests/imxrt1180-wm8962`). With the **Audio PLL now modelled** (see the CCM row), a SAI1 muxed to `AudioPllOut` gets a real 24.576 MHz MCLK — so the codec + PLL together are the control/clock half of the stock `sai/edma_transfer` streaming path |
| **ASRC** (sample-rate converter) | 1 | 0x429A0000 | ● data-path | `hw/audio/imxrt1180_asrc.c`. The m2m path the `fsl_asrc` `ASRC_TransferBlocking` driver drives: init-done poll (ASRCFG INIRQ), the ASRDIx→resampler→ASRDOx FIFO handshake (ASRSTR AIDEA/AODFA), IRQ 235. Converts at the in:out ratio `(outSrcHz/inSrcHz)·(in_period/out_period)`: the divider ratio from ASRCDR1 times the **true-async source factor** from the two ASRCSR-selected clocks. Same source (every m2m example) → factor 1; different SAIs → each SAIn TX bit clock is resolved via a link to the SAI (`imxrt1180_sai_tx_bclk_hz`) and the real factor applied (`tests/imxrt1180-asrc-async`, 1:2 upsample driven purely by the SAI2/SAI1 clock ratio, mutation-proven). Unresolvable sources (RX/SPDIF/clock-roots/MIC/MQS) are flagged, not fabricated. The conversion is a **REAL bandlimited resampling** — a polyphase windowed-sinc FIR (Hann-windowed 16-tap sinc, ratio-tracking cutoff, unity DC gain), NOT the crude linear interpolation it replaced. Proven against DSP first principles (`tests/imxrt1180-asrc`, mutation-proven): unity DC gain (const→const byte-exact) + Nyquist-tone stop-band rejection to ~-56 dB on a 2:1 down-conversion (linear interp would alias it to full-scale DC). NOT NXP's exact polyphase taps (unpublished → values won't bit-match hardware; flagged, not faked). The stock `asrc_m2m_polling` example **runs end-to-end** (both SAI playbacks + the 48k→32k convert; scorecard PASS) once the SAI TX FIFO drains at the codec's real fs rather than the ~100 Hz audio-callback cadence (see the **sai** row's `drain_timer`). IRQ 235 |
...
Today that is **LPUART1/2** (`CH_MUX[SRC]` 16–19), **SAI1–4 TX** (`SRC` 21 on
eDMA3; 180/182/184 on eDMA4 — the low byte of `kDmaNRequestMuxSaiKTx` in
`PERI_DMA4.h`), **LPSPI1/2 Tx+Rx** (`SRC` 11–14 on eDMA3), and **LPI2C1/2 Tx+Rx**
(`SRC` 7–10 on eDMA3).

The SAI asserts its TX line while `TCSR[FRDE]`+FRF (or `TCSR[FWDE]`+FWF) hold, and
the line falls of its own accord as the serviced TDR writes push the FIFO past the
watermark — value-verified end-to-end by `tests/imxrt1180-sai-dma` (2048 samples
mem→eDMA→SAI FIFO→wav, byte-exact, at two rates, CPU never touching TDR; FRDE gate
mutation-proven). The LPSPI asserts its TX line on `DER[TDDE]` (TX is synchronous,
so the level is always "wants data") and its RX line on `DER[RDDE]` + RX-FIFO above
`FCR[RXWATER]` — `tests/imxrt1180-lpspi-dma` reads a flash JEDEC ID full-duplex by
eDMA (mem→TDR and RDR→mem), byte-exact, CPU never touching TDR/RDR, with **both**
the TDDE and RDDE gates mutation-proven to refuse. The LPI2C is the same shape on
`MDER[TDDE]`/`[RDDE]` + `MFCR[RXWATER]`, except its TX stream is *command* words —
`tests/imxrt1180-lpi2c-dma` streams a register-read command sequence to MTDR by
eDMA and pulls the received bytes MRDR→mem, byte-exact vs a PIO golden (I2C
self-frames via START/STOP, so no CS wiring is needed), both gates mutation-proven.

The **LPADC** is the first **conversion-driven** source and the first on **eDMA4**
(`edma[1]`, SRC 57/158 — the `0x200`-tagged `kDma4RequestMuxADCn` lines): its
request rises when a result FIFO fills above `FCTRL[FWMARK]` (the same level event
as `STAT.RDYn`) gated by `DE[FWMDEn]`+`CTRL[ADCEN]`, and falls as the serviced
`RESFIFO` reads drain it. Single direction (result collection — no TX side).
`tests/imxrt1180-lpadc-dma` fires one SWTRIG (`CMDH.LOOP=7` → 8 distinct results)
and the eDMA4 drains all 8 RESFIFO entries to memory, byte-exact vs a PIO golden,
CPU never reading RESFIFO; the `FWMDE0` gate is mutation-proven.

The **eFlexPWM** value-DMA is the first **reload-event-driven** source: with
`DMAEN[VALDE]` set, each submodule reload asks the eDMA to refresh its VALx compare
registers for the next period (the FOC duty-update path). Wired to eDMA4 (SRC
62/70/78/86 for PWM1–4 SM0, +sm per submodule). There is no fill level — the line
is asserted at the reload tick and lowered by the VALx write that services it (one
minor loop per reload; a `qemu_irq_pulse` would vanish before the eDMA's bottom
half ran). Because VALx is double-buffered (commit on `MCTRL.LDOK`, not at reload),
`tests/imxrt1180-pwm-dma` lets the eDMA4 walk a ramp into VAL3 across 8 reloads,
then commits with a CPU LDOK and reads it back byte-exact — the real hardware flow,
no auto-commit shortcut; the `VALDE` gate is mutation-proven.

> ⚠️ **LPSPI chip-select is not wired to `-device`-attached SPI slaves.** The SoC
> connects the FlexSPI's CS but leaves the LPSPI `cs_lines` dangling, so an m25p80
> on an LPSPI bus never sees a CS deassert and cannot reframe: it answers a
> command (e.g. RDID) exactly ONCE per boot, then returns stream data. The PIO
> `tests/imxrt1180-lpspi` hides this by doing a single read; `-lpspi-dma` is
> structured around it (positive phase = the flash's first command). Wiring LPSPI
> CS to attached slaves is a separate gap — real, and not introduced by the DMA work.

Every other modelled block that is a DMA source on real silicon is **PIO-only
here**: **SAI1–4 RX** (RX FIFO not modelled), **eFlexPWM capture** (capture not
modelled), `FlexCAN` (message-FIFO DMA), `LPTMR1` (`SRC` 15 — a timer-compare event
with no serviced-write to lower the request, so it needs a DMA-ack model the eDMA
does not expose), `RGPIO`. Each needs new sub-block modelling, not just wiring.
Their register models are correct and their tests pass, and a driver that moves
data through them **by CPU** works — but a driver that configures eDMA and waits
for it will **wait forever**, because nothing ever asks. That is a hang, not a
silent wrong answer, so it is honest; it is still a gap.
...
- **asrc** ✅ (m2m data path modelled): the shared blockers were done first — the
  **Audio PLL** (`AUDIO_PLL.CTRL0/NUMER/DENOM` compute a real frequency; the CCM row)
  ...
  bandlimited anti-imaging/anti-aliasing filter, unity DC gain, cutoff tracking the
  ratio) — proven against DSP first principles (unity DC gain + Nyquist-tone stop-band
  rejection, `tests/imxrt1180-asrc`, mutation-proven), NOT NXP's exact polyphase taps
  (unpublished → sample values won't bit-match silicon; flagged, not faked). The stock
  `asrc_m2m_polling` now **runs end-to-end** (both SAI playbacks + the 48k→32k convert;
  scorecard PASS) — the earlier "hangs in the 2nd playback" was SAI TX *throughput*,
  not a rate-reconfig gap: the FIFO drained only on the ~100 Hz audio-backend callback,
  far below fs, so a large interrupt-driven transfer crawled.  Fixed by an fs-paced SAI
  drain timer (see the **sai** row).
