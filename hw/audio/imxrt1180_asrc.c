 ...
 * See the header for the rationale.  The resampler is a Hann-windowed-sinc polyphase
 * FIR (the CLASS the hardware uses, not NXP's unpublished taps) -- see asrc_winsinc.
...
static int vmstate_asrc_post_load(void *opaque, int version_id)
{
    asrc_update_irq(opaque);
    return 0;
}
...
static const VMStateDescription vmstate_asrc = {
    ...
    .post_load = vmstate_asrc_post_load,
