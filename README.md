...
- **[`docs/validation/SCORECARD.md`](docs/validation/SCORECARD.md)** → the tracked
  example scorecard: every row runs a real NXP SDK example and is MEASURED against
  an **external oracle** (a string the firmware itself prints, cited by SDK
  `file:line`), regenerated by `tools/scorecard.sh` behind a coverage gate. Honest
  classes — PASS / BANNER (blocked on an external host) / XFAIL (documented gap) /
  VALUE-PROVEN (pinned by a value-test) — never one collapsed headline.
...
- `docs/validation/` — the example scorecard (`corpus.tsv` manifest + `tools/scorecard.sh` generator + `SCORECARD.md`)
